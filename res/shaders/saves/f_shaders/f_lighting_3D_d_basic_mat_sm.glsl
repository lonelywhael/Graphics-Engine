...
vec3 getAmbient(vec3 lightAmbient) {
    vec3 ambientLight = material.ambient * lightAmbient;
    return ambientLight;
}
vec3 getDiffuse(vec3 lightDiffuse, vec3 norm, vec3 lightDir) {
    float diffuse = max(dot(norm, lightDir), 0.0);
    vec3 diffuseLight = diffuse * material.diffuse * lightDiffuse;
    return diffuseLight;
}
vec3 getSpecular(vec3 lightSpecular, vec3 norm, vec3 lightDir) {
    ...
    vec3 specularLight = specular * material.specular * lightSpecular;
