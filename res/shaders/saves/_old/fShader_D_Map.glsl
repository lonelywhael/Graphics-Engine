 ...
 * 
 * Almost identical to the basic material shader, this shader instead draws from a texture for ambient and diffuse values.
 */
...
uniform DiffuseMap dMap;
uniform Light lightList[N_LIGHTS];

vec3 getAmbient(float c, vec3 lightAmbient) {
    vec3 ambientLight = c * vec3(texture(dMap.diffuse, TexCoord)) * lightAmbient;           // use texture call here
    return ambientLight;
}
vec3 getDiffuse(float c, vec3 lightDiffuse, vec3 nNorm, vec3 lightDir) {
    float diffuse = max(dot(nNorm, lightDir), 0.0);
    vec3 diffuseLight = c * diffuse * vec3(texture(dMap.diffuse, TexCoord)) * lightDiffuse; // use texture call here
    return diffuseLight;
}
vec3 getSpecular(float c, vec3 lightSpecular, vec3 nNorm, vec3 lightDir) {
    ...
    vec3 specularLight = c * specular * dMap.specular * lightSpecular;                      // use normal material call here
